...
def build_engine_caffe(model_file, deploy_file,trans_layer,starts_gpu,batch):
    with trt.Builder(
        ...
        print("OUTPUT")
        for i, layer in enumerate(network):
            print(layer.name, " and i:",i ," shape:", network.get_layer(i).get_output(0).shape)
        # exit()
...
if __name__ == "__main__":
    ...
    if serialized_engine is not None:
        save_engine(serialized_engine.serialize(), str("vgg19_gpu_transition_at_"+transition+".plan"))

    exit()
    serialized_engine = build_engine_caffe(None, prototxt,1000,True,batch)
    if serialized_engine is not None:
        save_engine(serialized_engine.serialize(), str("baseline_DNNs/"+network_stem+"_dla.plan"))

    exit()
    prototxt_files = glob.glob(f"/home/ismetdagli/Transition-Trt/mobilenet_yolov3_lite_deploy.prototxt")
    ...
    if serialized_engine is not None:
        save_engine(serialized_engine.serialize(), str("baseline_DNNs/"+network_stem+"_gpu.plan"))
    ...
    for i in {0,10,24,38,53,67,81,95,109,124,141}:
        ...
        if serialized_engine is not None:
            save_engine(serialized_engine.serialize(), str("googlenet_batch2/"+network_stem+"_dla"+str(i)+"_batch2.plan"))
    
    for i in {0,10,24,38,53,67,81,95,109,124,141}:
        serialized_engine = build_engine_caffe(None, prototxt, i,False,batch)
        if serialized_engine is not None:
            save_engine(serialized_engine.serialize(), str("googlenet_batch2/"+network_stem+"_dla"+str(i)+"_batch1.plan"))
    ...
    for prototxt in prototxt_files:
        if count >= -1:
            ...
            print(network_stem)
            batch=2
            serialized_engine = build_engine_caffe(None, prototxt, 2000,starts_gpu,batch)
            if serialized_engine is not None:
                save_engine(serialized_engine.serialize(), str("multiple_batch_engines/"+network_stem+"_maxBatch2_dla.plan"))
            
            serialized_engine = build_engine_caffe(None, prototxt, 0,starts_gpu,batch)
            if serialized_engine is not None:
                save_engine(serialized_engine.serialize(), str("multiple_batch_engines/"+network_stem+"_maxBatch2_gpu.plan"))
        count+=1  
