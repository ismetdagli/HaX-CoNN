...
> Creating a docker or a VM is infeasible due to large size/access to the required hardware (DLA) etc. The authors provide remote access as explained below. 
...
The build can be done conveniently by the build_engine.py script. One thing to note is we need two types of engines. One with transitions and the other with a certain layer marked as output. The transitioning engine runs on both device whereas the marked engine runs on single device.

An engine which starts on GPU and transitions to DLA on layer 38:
```bash
python3 src/build_engine.py \
--prototxt prototxt_input_files/googlenet.prototxt \
--start gpu \
...
An engine which starts on DLA and marks the output as layer 38:
...
The generated profile is useful for layer analysis of GPU however the profile needs to be filtered from unnecessary information. Also, the fused layers do not provide an easily accesible layer count so we provided our own parsing and safe filtering for this profile.
...
- Then, run z3 solver (`src/dummy_z3_solver`) in an infinite loop and run the same executions of AlexnetDLA + GPU(any network). 
 >  `while(True)` at Line 128 on z3_solver helps to run in an infinite loop. You can define a automated stopping mechanism to stop the code after all executions done. (not necessary but optional. If we leave as it is, it should not be an issue)
