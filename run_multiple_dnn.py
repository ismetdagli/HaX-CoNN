...
warmUp=0
iteration=1000
def process_run(network_name,batch,warmUp,iteration,engine,output_dir,threadno,other_network,TRTEXEC_PATH,count):
    dt = datetime.now()
    print("iter:", dt)
    with open(output_dir+"/"+str(network_name)+"_"+str(other_network)+"_"+str(threadno)+"_results.log", 'w') as log_file:
        subprocess.run([TRTEXEC_PATH, "--iterations="+str(iteration),"--dumpProfile", "--batch="+str(batch),"--avgRuns="+str(avgRun), "--warmUp="+str(warmUp), "--duration=0", f"--loadEngine={engine}"], stdout=log_file)
...
for engine in input_engines:
    ...
    for engine_2 in input_engines_2:
        ...
        batch1=1
        batch2=1
        t1 = threading.Thread(target=process_run,args=(network_name,batch1,warmUp,iteration,engine,output_dir,1,network_name_2,"/home/ismetdagli/tensorrt_sharedMem1/bin/trtexec",count))
        t2 = threading.Thread(target=process_run,args=(network_name_2,batch2,warmUp,iteration-2,engine_2,output_dir_2,2,network_name,"/home/ismetdagli/tensorrt_sharedMem2/bin/trtexec",count))
        t1.start()
        time.sleep(1) #
        t2.start()
        t1.join()
